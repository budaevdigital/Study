# Для тестирования подключений: telnet 127.0.0.1 8000
...
def new_connection(selector: selectors.BaseSelector, sock: socket.socket):
    ...
    logger.info(f"accepted new_conn from {address}")
...
def run_interation(selector: selectors.BaseSelector):
...
def serve_forever():
    """
    Метод запускает сервер на постоянное прослушивание новых
    сообщений
    """
